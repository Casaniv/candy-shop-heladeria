...
class UI {
	...
	detalleProducto(id) {
		...
		filtroDato.forEach(item => {
			result += `
			<article class="detalle-grid">
				<img src=${item.imagen} alt="${item.producto}" class="img-fluid">
				<div class="detalles-content">
					...
					<div class="rating">
						...
						<p class="price"><b>Precio: </b> S/.${item.precio}</p>
						<p class="description">
							<b>Descripcion:</b> <span>${item.descripcion}</span>
						</p>

						<div class="bottom">
							<div class="btn__group">
								<button class="btn addToCart" data-id=${item.id}>Añadir carrito</button>
							</div>
						</div>
				</div>
			</article>
	...
	renderProductos(inventario) {
		let result = ""
		inventario.forEach((item) => {
			result += `
			<div class="producto">
				<div class="image__container"><img src=${item.imagen} alt="${item.producto}"></div>
				...
				<div class="bottom">
					<div class="btn__group">
						<button class="btn addToCart" data-id=${item.id}>Añadir carrito</button>
						<a href="producto-detalles.html?id=${item.id}" class="btn view">Vista</a>
					</div>
				</div>
	...
	getButtons() {
		...
		buttons.forEach((button) => {
			...
			const inCart = carrito.find(item => item.id === parseInt(id));
	...
	setItemValues(carrito) {
		...
		carrito.map(item => {
			...
			tempTotal += parseFloat(item.precio) * parseInt(item.cantidad);
	...
	addCarritoItem({ imagen, precio, producto, id }) {
		...
		<div>
			<span class="increase" data-id=${id}><i class="bx bxs-up-arrow"></i></span>
			<p class="item__cantidad">1</p>
			<span class="decrease" data-id=${id}><i class="bx bxs-down-arrow"></i></span>
		</div>

		<div><span class="remove__item" data-id=${id}><i class="bx bx-trash"></i></span></div>
		`
...
// 	async getProductos() {
// 		try {
// 			const productos = document.addEventListener();
// 			return productos
// 		} catch (err) {
// 			console.log(err)
// 		}
...
function categoryValue() {
	...
	if (category.length > 0) {
		const producto = productos.filter(item => item.categoria === category)
...
Papa.parse(stringURLs.inventario, {
	download: true,
	header: true,
	dynamicTyping: true,
	complete: function (results, file) {
		console.log("Parsing complete:", results, file);
		data.inventario = results.data;
